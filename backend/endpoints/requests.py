...
def put_request() -> tuple:
    ...
    data = request.json
    try:
        if all(key in data for key in ['id', 'user_id', 'image_id', 'reason', 'status', 'created_at']):
            request_dict = {
                'id': data['id'],
                'user_id': data['user_id'],
                'image_id': data['image_id'],
            ...
            return jsonify({"message": "Missing key(s) 'id', 'user_id', 'image_id', 'reason', 'status', 'created_at'"}), 400
    except Exception as e:
            return jsonify({"message": f"An error occurred: {str(e)}"}), 500
